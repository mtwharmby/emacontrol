...
def robot_begin():
    ...
    sam_pos = ema.get_sample_number()

    if sam_pos != 1:  # FIXME Check flag; check powerstate
        ema.sample_index = sam_pos  # FIXME This shoudn't be needed...?
        # TODO Log: 'Sample coords at robot start are ({}, {}) (Sample {}).
        # Should be (0, 0) for Sample 1'.format(coords['X'], coords['Y'],
        # sample)
        print('WARNING: Current sample is {} (not 1!).'.format(sam_pos))
